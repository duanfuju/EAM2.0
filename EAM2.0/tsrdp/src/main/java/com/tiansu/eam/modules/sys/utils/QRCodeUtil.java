...
public class QRCodeUtil {
    ...
    public static BufferedImage createQRCodeWithLogo(String data,
        ...
        try {
            BufferedImage qrcode = createQRCode(data, charset, hint, width,
                    height);
            if(!logoFile.exists()){
                logoFile = new File(Servlets.getRequest().getSession().getServletContext().getRealPath("")+"/WEB-INF/views/img/eam/qrcode_demo.jpg");//throw new RuntimeException("文件不存在!");
            }
    ...
    public static void convertBase64StringToImage(String base64ImageString,
        ...
        try {
            Base64 d = new Base64();
            byte[] bs = (byte[]) d.decode(base64ImageString);
