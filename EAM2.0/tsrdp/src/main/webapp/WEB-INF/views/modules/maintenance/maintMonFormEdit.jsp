        ...
        $(function () {
            ...
            var tools = $("#tools").ligerGrid({
                columns: [
                    ...
                    { display: '数量', name: 'tool_num', type: 'int', editor: { type: 'int'} },
                    ...
                    { display: '备注', name: 'tool_remark',editor: { type: 'text' },editor: { type: 'text' }},
                    { display: '操作', isSort: false,
                        render: function (rowdata, rowindex, value)
                        {
                            var h = "";
            ...
            var spareparts = $("#spareparts").ligerGrid({
                columns: [
                    ...
                    { display: '数量', name: 'attachment_num', type: 'int', editor: { type: 'int'} },
                    { display: '计量单位', name: 'material_unit',
                        render: function (rowdata, rowindex, value) {
                            var material_unit;
                            $.each(parent.materialSelect, function (i, data) {
                    ...
                    { display: '备注', name: 'attachment_remark', type: 'text', editor: { type: 'text'} },
                    { display: '操作', isSort: false,
                        render: function (rowdata, rowindex, value)
                        {
                            var h = "";
            ...
            var person = $("#person").ligerGrid({
                columns: [
                    ...
                    { display: '额定工时', name: 'emp_hour', type: 'float', editor: { type: 'float'} },
                    { display: '额定工时单价', name: 'emp_price', type: 'float', editor: { type: 'float'} },
                    ...
                    { display: '岗位技能', name: 'emp_skills',editor: { type: 'text' }},
                    { display: '备注', name: 'emp_remark',editor: { type: 'text' }},
            ...
            var otherexpenses = $("#otherexpenses").ligerGrid({
                columns: [
                    { display: '其他费用事项', name: 'charge_name', type: 'text', editor: { type: 'text'} },
                    { display: '金额', name: 'charge_price', type: 'float', editor: { type: 'float'} },
                    { display: '备注', name: 'charge_remark',type: 'text',editor: { type: 'text' }},
                    { display: '操作', isSort: false,
                        render: function (rowdata, rowindex, value)
                        {
                            var h = "";
            ...
            $("input[name='project_device']").on("click",function () {
                layer.open({
                    type: 2,
                    skin: 'layui-layer-rim', //加上边框
                    area: common.getAreaInner(), //宽高
                    closeBtn: 1, //显示关闭按钮
                    content: "/a/maintain/maintainProjectSub/DeviceSelectUI"
                });
            });
//给编辑页面字段赋值
            ...
            common.callAjax('post',false,ctx + "/maintain/maintainProjectSub/editObj","json",{"id" : parent.editId},function(data){
                ...
                //巡检周期
                    ...
                    $('.showpick div.showpickdiv').eq(data.project_period).css('display','block').siblings().css('display','none');
						...
						$('#project_empidBox').html(parent.personSelect);//保养责任人
						...
						$('#deviceId')	.val(data.project_device);
						$('input[name="project_stime"]').val(DateUtil.dateToStr('yyyy-MM-dd',new Date(data.project_stime)));
            });

            $('#inputForm').on('change','#project_isoperationBox',function(){
                if($(this).val()=='0'){
                    liger.get('inputForm').setVisible('project_opework',false);
				}else{
                    liger.get('inputForm').setVisible('project_opework',true);
                    FormUtil.initSelectBox(ctx+"/eam/operationwork/getApprovedWorkCodes","standCode",$("#project_opeworkBox"),null,null,"id","code");
                }
            })
            //巡检周期change事件
            ...
            $("#btnSubmit").on("click",function () {
                ...
                if (!form.valid()) {
                    ...
                    sendObj.project_cycleyear=$('input[name="project_time"]').val();
                    sendObj.project_device=$('#deviceId').val();
                    sendObj.project_code=$('input[name="project_code"]').val();
					sendObj.project_name=$('input[name="project_name"]').val();
                    ...
                    sendObj.route_isstandard=$('#project_isoperationBox').val();
                    sendObj.project_empid=$('#project_empidBox').val();
                    sendObj.maintSubProcs=procedure;
                    sendObj.maintSubSafes=safety;
                    sendObj.maintSubTools=tools;
                    sendObj.maintSubSpareparts=spareparts;
                    sendObj.maintsubPersons=person;
                    sendObj.maintsubOthers=otherexpenses;
					console.log(sendObj);
                    common.callAjax('post',false,'${ctx}/maintain/maintainProjectSub/update',"text",{param:JSON.stringify(sendObj)},function(data) {
                        if (data == "success") {
