...
//            $('#statusBox').val(['1']).trigger('change');
            //设置隐藏
//            $("#project_bmBox").append($("<input></input>").attr("type", "hidden").attr("name", "project_bm"));
            ...
            //保养计划编码
            //获取编码
            common.ajaxForCode({type:"MAINT_SET"},false,"text",function(data){
                if (data !="" ||data !=null) {
                    console.log(data);
                    project_code=data;

                    $("input[name='project_code']").val(data);
                }
            })

//设置默认值
            $("#statusBox").val(1).trigger('change');
//            $("input[name='statusBox']").val(1)
...
//                    alert("只剩一项时无法删除");
                    layer.msg("只剩一项时无法删除",{time: 1000,icon:2});
                }else {
                ...
                if (!form.valid()) {
                    ...
                    if($('#project_periodBox').val()=='0'){
                        ...
                        if(periodDetail==""||periodDetail==null){
                            layer.msg("请填写周期",{time: 1000,icon:2});
                            return;
                        }
                    ...
                    console.log($('input[name="project_code"]').val());
                    sendObj.project_code=project_code;
                    sendObj.project_name=project_code;
...
//              sendObj.route_object=$('input[name="route_object"]').val();
                   ...
                   //
                    ...
                    common.callAjax('post',false,'${ctx}/maintenance/maintSet/insert',"text",{param:JSON.stringify(sendObj)},function(data) {
                        if (data == "success") {
