...
define(["text!modules/supplier/supplierType.html", "text!modules/supplier/supplierType.css","liger-all"], function (htmlTemp, cssTemp) {
       ...
       init:function(menuno){
        ...
        initFieldAuth:function (id) {
            ...
            common.callAjax('post',false,ctx + "/material/testMaterial/getfields","json",param,function(data){
                ...
                $.each(searchformConfig,function(index,item){
                    if(item.name=='type_status'){
                        item.option={data:status};
                    }
                });
                //列表赋值
        ...
        initButtonForm : function (id) {
                        ...
                        if(btn.buttonname=="编辑"){
                            ...
                            obj.callBack=function (data) {
                                ...
                                layer.open({
                                    ...
                                    closeBtn: 1, //显示关闭按钮
                                    content: ctx+"/supplier/supplierType/editUI"
                                });
                            };
                            btnInline.push(obj);
                            ...
                            obj.callBack=function (data) {
                                        layer.confirm('确认删掉这条数据吗？',{icon: 3, title:'提示'},function() {
                                            common.callAjax('post', false, ctx + "/supplier/supplierType/delete?id=" + data.id_key, "text", null, function (d) {
                                                if (d == "success") {
                            ...
                            obj.callBack=function (data) {
                                ...
                                layer.open({
                                    ...
                                    area: common.getAreaXS(), //宽高
                                    closeBtn: 1, //显示关闭按钮
                                    content: "/a/supplier/supplierType/detailUI"
                                });
                            };
                            btnInline.push(obj);
                        }
            ...
            $("#add").on("click", function () {
                layer.open({
                    ...
                    area: common.getAreaXS(), //宽高
                    closeBtn: 1, //显示关闭按钮
                    content: ctx+"/supplier/supplierType/addUI"
                });
            });
        ...
        initTableForm : function () {
            ...
            var tab = common.renderTable("mytable", {
                ...
                "opBtns": btnInline,
                "ajax": {
                    "url":ctx+ '/supplier/supplierType/listData',
            ...
            $("#delete").on("click",function () {
                ...
                layer.confirm('确认删掉这条数据吗？',{icon: 3, title:'提示'},function() {
                    ...
                    common.callAjax('post', false, ctx + "/supplier/supplierType/delete?id=" + ids, "json", param, function (data) {
                        if (data == "success") {
                            layer.msg('删除成功！', {icon: 1,time: 1000}, function (index) {
                                $('#mytable').dataTable().fnDraw(false);
                                layer.close(index);
            ...
            $("#fileField").on("change",function () {
                common.setExcel('supplierType');
            });
            //下载模板
            $("#download").on("click",function () {
                common.loadExcelModel("supplierType");
            });
            ...
            $("#export").on("click",function () {
                ...
                window.open('/exportExcel.do?ids='+(ids.substr(0,ids.length - 1))+'&name=supplierType');
            });
            //全部导出
            $("#exportall").on("click",function () {
                window.open('/exportExcel.do?name=supplierType');
            });
