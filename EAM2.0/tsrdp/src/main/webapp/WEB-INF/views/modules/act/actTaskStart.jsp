            ...
            $("#inputForm").on("submit", function () {
                ...
                $(this).ajaxSubmit({
                    ...
                    success: function (data) {
                        if (data.length>0) {
                            layer.msg('提交成功！'+data,{icon:1,time: 1000}, function (index) {
...
<body>
	...
	<hr/>
	<form action="${ctx }/eam/act/process/startProcessInner?pdKey=${processDefinition.key}" id="inputForm" class="form-horizontal" method="post">
		<c:if test="${hasStartFormKey}">
		${startFormData}
		</c:if>
		...
		<div class="control-group">
			<div class="controls">
				...
				<button type="submit" class="btn" id="btnSubmit"><i class="icon-play"></i>启动流程</button>
			</div>
		</div>
	</form>
