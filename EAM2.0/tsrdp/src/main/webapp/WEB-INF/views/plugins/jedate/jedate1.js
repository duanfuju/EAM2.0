...
(function (root, factory) {
    ...
    jeDt.setHtml = function () {   //console.log("20160906".substr(4))   //console.log("20160906".replace(/^(\d{2})(?=\d)/g,"$1,"))
        var weekHtml = "", tmsArr = "", date = new Date(), dateFormat = jeDt.checkFormat(jeDt.format),
            isYYYY = dateFormat == "YYYY" ? true : false,
        ...
        var datetopStr = (dateFormat == "YYYY" ? '<div class="jedatetop" style="display:none">' : '<div class="jedatetop">') + (!isYYMM ? '<div class="jedateym" style="width:29%;margin-left:55px;margin-right:20px;"><i class="prev triangle yearprev"  ></i><span class="jedateyy" ym="24"><em class="jedateyear"></em><em class="pndrop" style="display:none;"></em></span><i class="next triangle yearnext"></i></div>' + '<div class="jedateym" style="width:29%;"><i class="prev triangle monthprev" ></i><span class="jedatemm" ym="12"><em class="jedatemonth"></em><em class="pndrop" style="display:none;"></em></span><i class="next triangle monthnext" ></i></div>' : '<div class="jedateym" style="width:50%;margin:0px 25%;"><i class="prev triangle ymprev"></i><span class="jedateyy"><em class="jedateyearmonth"></em></span><i class="next triangle ymnext"></i></div>') + "</div>";
        var dateymList = !isYYMM ? '<div class="jedatetopym" style="display: none;">' + '<ul class="ymdropul"></ul><p><span class="jedateymchle">&lt;&lt;</span><span class="jedateymchri">&gt;&gt;</span><span class="jedateymchok">关闭</span></p>' + "</div>" : (dateFormat == "YYYY" ? '<ul class="jedayy" style="height:40px;"></ul><p><span class="jedateymchle" style="display: block;   position: absolute; top: 0px;left:0px;cursor: pointer"><img src="../../../js/plugin/jedate/img/right1.png" style="transform: rotate(180deg);width:8px;"></span><span class="jedateymchri" style="display: block;  float: right; position: absolute; top: 0px; right: 10px;cursor: pointer;"><img src="../../../js/plugin/jedate/img/right1.png" style="width:8px;"></span><span class="jedateymchok" style="display: none;">关闭</span></p>' : '<ul class="jedaym"></ul>');
        ...
        var datebotStr = (dateFormat == "YYYY" || jeDt.opts.isCondition == false ? '<div class="jedatebot" style="display:none">' : '<div class="jedatebot">') + bothmsStr + "</div>";
        ...
        if (isYYMM) {
            ...
            if (dateFormat == "YYYY") {
                ...
                jeDt.attr(monthCls, "data-onym", tmsArr[0]);
                jeDt.text(monthCls, tmsArr[0]);
    ...
    jeDt.createDaysHtml = function (ys, ms) {
        ...
        var year = parseInt(ys), month = ms, dateHtml = "", count = 0;
    ...
    jeDt.onlyYMStr = function (y, m) {
        ...
        jeDt.each(jeDt.montharr, function (i, val) {
            ...
            var minArr = jeDt.parseMatch(jeDt.minDate), maxArr = jeDt.parseMatch(jeDt.maxDate),
                thisDate = new Date(y, jeDt.digit(val), "01"), minTime = new Date(minArr[0], minArr[1], minArr[2]), maxTime = new Date(maxArr[0], maxArr[1], maxArr[2]);
            if (thisDate < minTime || thisDate > maxTime) {
                onlyYM += "<li class='disabled' ym='" + y + "-" + jeDt.digit(val) + "'>" + jeDt.digit(val) + "</li>";
            } else {
                onlyYM += "<li " + (m == val ? 'class="action"' : "") + ' ym="' + y + "-" + jeDt.digit(val) + '">' + jeDt.digit(val) + "</li>";
            }
        });
        jeDt.text(jeDt.find(".jedateym .jedateyearmonth")[0], y);
        return onlyYM;
    };
    //循环生成年（YYYY）
    jeDt.onlyYear = function (YY) {
        ...
        var dateFormat = jeDt.checkFormat(jeDt.format)
        if (dateFormat == "YYYY") {
            var onlyStr = "";
            jeDt.yearArr = new Array(3);
            var jedateymchri = jeDt.find(".jedateymchri");
            jeDt.each(jeDt.yearArr, function (i) {
                var minArr = jeDt.parseMatch(jeDt.minDate), maxArr = jeDt.parseMatch(jeDt.maxDate),
                    minY = minArr[0], maxY = maxArr[0], yyi = YY - 2 + i,
                    getyear = jeDt.attr(jeDt.find(".jedateym .jedateyearmonth")[0], "data-onyy");
                if (yyi < minY || yyi > maxY) {
                    onlyStr += "<li class='disabled' yy='" + yyi + "'>" + yyi + "</li>";
            ...
            jeDt.bind(jedateymchri, "click", function (ev) {
                var listr = '';
                for (var i = 0; i < jeDt.yearArr.length; i++) {
                    var minArr = jeDt.parseMatch(jeDt.minDate), maxArr = jeDt.parseMatch(jeDt.maxDate),
                        minY = minArr[0], maxY = maxArr[0], yyi = YY - 2 + i,
                        getyear = jeDt.attr(jeDt.find(".jedateym .jedateyearmonth")[0], "data-onyy");
                    onlyStr = '';
                    yyi += 3;
                    if (yyi < minY || yyi > maxY) {
                        listr += "<li class='disabled' yy='" + yyi + "'>" + yyi + "</li>";

                    } else {
                        listr += "<li " + (getyear == yyi ? 'class="action"' : "") + " yy='" + yyi + "'>" + yyi + "</li>";
                    }


                }
                jeDt.html(jeDt.find(".jedayy")[0], listr);
                YY = parseInt(YY) + 3;

                jeDt.ymPremNextEvents();

            })
            var jedateymchle = jeDt.find(".jedateymchle");
            jeDt.bind(jedateymchle, "click", function (ev) {
                var listr = '';
                for (var i = 0; i < 3; i++) {
                    var minArr = jeDt.parseMatch(jeDt.minDate), maxArr = jeDt.parseMatch(jeDt.maxDate),
                        minY = minArr[0], maxY = maxArr[0], yyi = YY - 2 + i,
    ...
    jeDt.chooseDays = function () {
        jeDt.bind(jeDt.find(".jedaul li"), "click", function (ev) {
            ...
            /*重新封装添加start*/
            console.log(jeDt.opts.isClose == !undefined);
            if (jeDt.opts.isClose == undefined) {
                jeDt.dateClose();
            }
            /*重新封装添加end*/
    ...
    jeDt.ymPremNextEvents = function () {
        ...
        /* jeDt.bind(ymCls, "mousemove", function (ev) {
         var that = this;
         jeDt.each(ymCls, function(j, cls) {
