...
function getFormData(name) {
    var  data= {};
    for (var i = 0;i<formCopy.fields.length;i++){
        if (formCopy.fields[i].comboboxName == name){
            if(formCopy.fields[i].option) {
                data = formCopy.fields[i].option;
            }else{
                data = formCopy.fields[i].options;
            }
            data.readonly = formCopy.fields[i].readonly;
        }
    }
    return data;
};
function getClick(node,name){
    for (var i = 0;i<formCopy.fields.length;i++){
        if (formCopy.fields[i].name == name){
            var getonClick = formCopy.fields[i].options.tree.onClick;
            if(getonClick){
                getonClick(node);
            }
        }
...
function getZtreeData(name){
    var ztreeNode = {};
    for (var i = 0;i<formCopy.fields.length;i++){
        if (formCopy.fields[i].name == name){
            var url = formCopy.fields[i].options.tree.url;
            common.callAjax('post',false,url,"json",null,function(data){
                ztreeNode = data;
            })
        }
    }
...
function getQrcodeData(name) {
    var  data= null;
    for (var i = 1;i<formCopy.fields.length;i++){
        if (formCopy.fields[i].name == name){
            if(formCopy.fields[i].option) {
                data = formCopy.fields[i].option.data;
            }else{
                data = formCopy.fields[i].options.data;
            }
        }
    }
    return data;
}
function isTimeControl(name) {
    var status =null;
    for (var i = 1;i<formCopy.fields.length;i++){
        if (formCopy.fields[i].name == name){
            if(formCopy.fields[i].begintime){
                status = "start";
                formCopy.fields[i].begintime=false;
            }else if(formCopy.fields[i].endtime){
                status = "end";
            }
        }
    }
...
(function ($)
{
        ...
        var defaultOp = {
            ...
            getValue: function (editor, editParm)
            {
                ...
                if (editor.getValue)
                {
                    ...
                    if (field.type == "select"){
                        value = $("#"+field.comboboxName).val();
                    }
                    // if(field.type == "ztree"){
                    //     value = $("input[name='" + field.name +"']").attr('data-id');
                    // }
                    return value;
                }
            },
            getDisplay: function (editor, editParm)
            {
                //debugger;
                var field = editParm.field || editParm.column;
                if (editor.getValue)
                {
                    ...
                    if (field.type == "select"){
                        if ($("#"+field.comboboxName +" option:checked").text()!="请选择") {
                            value = $("#" + field.comboboxName + " option:checked").text();
                        }
                    }
            ...
            setValue: function (editor, value, editParm)
            {
                ...
                if (editor.setValue)
                {
                    ...
                    //修改引入BUG，修复中
                    if(editParm.field != undefined) {
                        if (editParm.field.type == "select") {
                            $("#" + editParm.field.comboboxName).val(value).trigger("change");
                            return;
                        }
                        if (editParm.field.type == "ztree") {
                            $("input[name='" + field.name +"']").val(value).trigger("change");
                            return;
                        }
                    }
                } 
    ...
    liger.editors = {
        ...
        "time":{
            control: 'TextBox',
            options: { timeEditor : true }
        },
        "dateTime":{
            control: 'TextBox',
            options: { dateTimeEditor : true }
        },
        "select":{
            control: 'TextBox',
            options: { selectEditor : true }
        },
        "ztree":{
            control: 'TextBox',
            options: { ztreeEditor : true }
        },
        "label":{
            control: 'TextBox',
            options: { labelEditor : true }
        },
        "qrcode":{
            control: 'TextBox',
            options: { qrcodeEditor : true }
        },
        "labelText":{
            control: 'TextBox',
            options: { labelTextEditor : true }
        },
    };
    liger.editors["string"] = liger.editors["text"];
    liger.editors["selectGrid"] = liger.editors["combobox"];
                    ...
                    if (g.trigger('beforeSetData', [data]) == false)
{
        ...
        selectValueByTree: function (value)
        {
            ...
            if (value != null)
            {
                var text = g.treeSelectInit(value) == "null"? "" :g.treeSelectInit(value);
...
(function ($)
{
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        getData: function (e)
        {
            ...
            function getFieldValueToData(fields, tabIndex)
            {
                $(fields).each(function (fieldIndex, field)
                {
                    ...
                    {
                        if (e=="display"){
                            var value = editor.editor.getDisplay.call(g, editor.control, {
                                field: field
                            });
                            g._setValueByName(g.data, display, value);
                        }else {
                            var value = editor.editor.getValue.call(g, editor.control, {
                                field: field
                            });
        ...
        setData: function (data)
        {
            ...
            function setDataToFields(fields, tabIndex)
            {
                $(fields).each(function (fieldIndex, field)
                {
                    ...
                    if (!editor) return;
                    if(g.data == "empty"){
                        editor.editor.setValue.call(g,editor.control, "", {
                            field: field
                        });
                        // if (field.type == "combobox") {
                        //     editor.control.inputText.val("");
                        //     editor.control.valueField.val("");
                        // }else{
                        //     editor.editor.setValue.call(g,editor.control, "", {
                        //         field: field
                        //     });
                        // }
                    }else if (name && (name in g.data))
                    {
        ...
        setFieldValidate: function (name, validate, messages)
        {
            ...
            {
                //验证包括必填规则，添加*
                jele.parents("li:first").next("li:first").append('<span class="l-star">*(必填)</span>');
            }
        },
        //提示 验证错误信息
        ...
        _buliderSpaceContainer: function (field)
        {
            ...
            {
                out.push("<span class='l-star'>*(必填)</span>");
            }
            out.push('</li>');
            return out.join('');
        },
    ...
    function getInvalidInf(errorList)
    {
        ...
        $(errorList).each(function (i, error)
        {
            ...
            if(!label){
                label = $(error.element).closest("ul").find("li").html();
            }
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [10, 15, 30, 100],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数 
        columns: [],                          //数据源
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm,sourceType)
        {
            ...
            if (p.dataAction == "server")
            {
                ...
                if (p.sortName)
                {
                    param.push({ name: 'columns[0][data]', value: p.sortName });
                    param.push({ name: 'order[0][column]', value:0 });
                    param.push({ name: 'order[0][dir]', value: p.sortOrder });
                    // param.push({ name: p.sortnameParmName, value: p.sortName });
                    // param.push({ name: p.sortorderParmName, value: p.sortOrder });
                }
                //param.push({ name: 'search[material_code]', value: '366' });
                if (p.parms instanceof Array) {
                    p.parms.forEach(function (e) {
                        param.push(e);
                    })
                }
                // param.push(p.parms);
        ...
        loadServerData: function (param, clause, sourceType)
        {
            ...
            var ajaxOptions = {
                ...
                success: function (data)
                {
                    //data.recordsTotal,表示调用的datatable格式的接口，将获取的
                    //数据转换为ligerGrid所需格式
                    if (data !=null) {
                        if (!data.Total) {
                            data = {Rows: data.data, Total: data.recordsTotal};
                        }
                    }
        ...
        */
        ...
        _showData: function (sourceType)
        {
            ...
            if($(".l-selected").length == parseInt($("select[name='rp']").eq(0).val()) *2){
                ...
                $(".l-grid-hd-row").eq(0).removeClass("l-checked");
            }
        },
        _fixHeight: function ()
        {
        ...
        _setEvent: function ()
        {
            ...
            $(document).bind("mouseout.grid", function (e)
            {
                //|| $('.l-grid-row-cell').has('.l-grid-row-cell-editing')
                // debugger;
                var flag=false;
                $('.l-box-select-absolute').each(function(){
                    if($(this).css("display")=='block'){
                        flag=true;
                    }
                })
                if($("#ui-datepicker-div").css('display')=='block'|| flag )
                    return;
                g.endEdit();
            });
...
(function ($)
{
    ...
    $.ligerDefaults.TextBox = {
        ...
        selectEditor:false,
        ztreeEditor:false,
        dateTimeEditor:false,
        labelEditor:false,
        qrcodeEditor:false,
        labelTextEditor:false,
        option:null
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            g.formatValue();
            if(p.timeEditor){
                if(isTimeControl(this.element.name)=='start'){
                    $(this.element).addClass('start_time_control').timepicker({
                        hourGrid: 4,
                        minuteGrid: 10,
                        onClose: function(selectedDate) {
                            $(".end_time_control").datetimepicker("option", "minDateTime", new Date(selectedDate));
                        }
                    });
                }else if(isTimeControl(this.element.name)=='end'){
                    $(this.element).addClass('end_time_control').timepicker({
                        hourGrid: 4,
                        minuteGrid: 10,
                        onClose: function(selectedDate) {
                            $(".start_time_control").datetimepicker("option", "minDateTime", new Date(selectedDate));
            ...
            if(p.dateEditor){
                if(isTimeControl(this.element.name)=='start'){
                    $(this.element).addClass('start_time_control').datepicker({
                        dateFormat: 'yy-mm-dd',
                        changeMonth: true,
                        changeYear:true,
                        yearRange:"1900:2100",
                        minDate:0,
                        onClose: function(selectedDate) {
                            $(".end_time_control").datepicker("option", "minDate", selectedDate);
                        }
                    });
                }else if(isTimeControl(this.element.name)=='end'){
                    $(this.element).addClass('end_time_control').datepicker({
                        dateFormat: 'yy-mm-dd',
                        changeMonth: true,
                        changeYear:true,
                        yearRange:"1900:2100",
                        minDate:0,
                        onClose: function(selectedDate) {
                            $(".start_time_control").datepicker("option", "maxDate", selectedDate);
                            $(".end_time_control").val($(this).val());
                        }
                    });
                }else{
            ...
            if(p.dateTimeEditor){
                if(isTimeControl(this.element.name)=='start'){
                    $(this.element).addClass('start_time_control').datetimepicker({
                        showSecond: true,
                        dateFormat: 'yy-mm-dd',
                        timeFormat: 'HH:mm:ss',
                        changeMonth: true,
                        ...
                        yearRange:"1900:2100",
                        onClose: function(selectedDate) {
                            var clearflag = $(".end_time_control").val();
                            $(".end_time_control").datetimepicker("option", "minDateTime", new Date(selectedDate));
                            $(".end_time_control").datetimepicker("option", "minDate", selectedDate);
                            if(!clearflag){
                                $(".end_time_control").val('')
                            }
                        }
                    });
                }else if(isTimeControl(this.element.name)=='end'){
                    $(this.element).addClass('end_time_control').datetimepicker({
                        showSecond: true,
                        minDate:0,
                        dateFormat: 'yy-mm-dd',
                        timeFormat: 'HH:mm:ss',
                        ...
                        yearRange:"1900:2100",
                        onClose: function(selectedDate) {
                            var clearflag = $(".start_time_control").val();
                            $(".start_time_control").datetimepicker("option", "maxDateTime", new Date(selectedDate));
                            $(".start_time_control").datetimepicker("option", "maxDate", selectedDate);
                            if(!clearflag){
                                $(".start_time_control").val('')
                            }
                        }
                    });
                }else{
            ...
            if(p.labelEditor){
                $(this.element).closest("li").next().remove();
                $(this.element).closest("li").before('<li><a>点击展开</a></li>');
                $(this.element).closest("li").remove();
            }
            if(p.qrcodeEditor){
                $(this.element).closest(".l-fieldcontainer").before('<div class="QR-code-area" style="height: auto !important;">' +
                        '<div>' +
                        '<!-- 图片 -->' +
                            ...
                            '<div class="QR-code" style="text-align:center;">' +
                                '<img id="ewm_display" src="" alt="" >' +
                            '</div> ' +
                            '<!--设备名称和设备编码文字 -->' +
                            '<div class="eqpInfo-txt pt10">' +
                                '<p class="QR-code-area-txtP1">设备名称:' +
                                    '<span class="ewmmc_display">asdsadas</span>' +
                                '</p> ' +
                                '<p class="QR-code-area-txtP2">设备编码:' +
                                        '<span class="ewmcode_display">12312312321313454323</span>' +
                                '</p>' +
                            '</div> ' +
                        '</div><!-- 底部阴影条 --> ' +
                    '</div>');
                $(this.element).closest(".l-fieldcontainer").closest('ul').addClass('QR-code-ul');
                $(this.element).closest(".l-fieldcontainer").remove();
                var data = getQrcodeData(this.element.name);

                document.getElementById("QR-code-area-logo").src =data.logo ;//上方logo
                document.getElementById("ewm_display").src =data.qrcode;//二维码
                // document.getElementById("QR-logo").src = xxx.jpg;//下方logo
                $(".QR-code-area-txt").html(data.title);//标题
                $(".ewmmc_display").text(data.equipname);//设备名称
                $(".ewmcode_display").text(data.equipcode);//设备编码
            }
            if(p.labelTextEditor){
                $(this.element).parent().removeClass("l-label");
                $(this.element).parent().addClass("l-label-text");
            }
            if(p.selectEditor){
                $(this.element).closest("li").next().css('float','right');
                $(this.element).closest("li").after("<select id='"+p.id+"'></select>");
                $(this.element).closest("li").remove();
                $("#"+p.id).select2({
                    allowClear: true,
                    above: false,
                ...
                var data = getFormData(this.element.name);
                //select2渲染的方法
                var optionHtml = "<option value=''>请选择</option>";
                data.id = data.id ||("id");
                data.text = data.text ||("text");
                $.each(data.data, function (i, item) {
                   optionHtml += "<option value=\"" + item[data.id] + "\">" + item[data.text] + "</option>";
                });
                $("#"+p.id).html(optionHtml);
                //新增隐藏域
                $("#"+p.id).append($("<input></input>").attr("type", "hidden").attr("name", p.valueFieldID));
                $("#"+p.id).on('change',function () {
                    $("input[name='" + p.valueFieldID +"']").val($("#"+p.id).val());//select2赋值
                })
                if(data.readonly)
            ...
            if(p.ztreeEditor){
                var that = this;
                $('#'+that.element.id).after($('<input>').attr('type','hidden').attr('name',p.id));
                $(this.element).attr('readonly','readonly')
                $('form').after("<div class='ztreeContent ztreeInputCont zTreeLeft "+p.id+"' style='display:none;z-index: 1001;background: #ffffff;border: 1px solid #15c7bd;border-top: none;width:200px;height: 200px;position: absolute; overflow:auto;'>"+
                "<input class='searchZtree' style='margin-left: 5px;width:170px;height:24px;'><i class='search-bt' style='position: absolute;top: 4px;left: 153px;'></i>"+
                "<ul id='"+p.id+"' class='ztree'></ul></div>");
                var treeNode = getZtreeData(that.element.name);
                var setting = {
                    view: {
                        showIcon: false,
                        fontCss: getFontCss
                ...
                $(this.element).click(function (e) {
                    ...
                    if($('.'+p.id).css('display') != 'none'){
                        $('#'+that.element.id).parent().css('border','1px solid #15c7bd')
                    }else{$('#'+that.element.id).parent().css('border','1px solid #999999')}
                    showMenu();
                });

                $('.'+p.id).css('margin-top',0);

                $(document).click(function(e){
                    e = window.event || e;
                    var obj = e.srcElement || e.target;
                    if($(obj).parents('.ztreeContent').length==0 && !$(obj).is('.ztreeContent') && !$(obj).is('input[name='+p.id+']')){
                        $('.'+p.id).hide();
                    }
                    if($('.'+p.id).css('display') != 'none'){
                        $('#'+that.element.id).parent().css('border','1px solid #15c7bd')
                ...
                //双击某个节点 然后将该节点的名称赋值值文本框
                function zTreeOnDblClick(e, treeId, treeNode) {
                    var zTree = $.fn.zTree.getZTreeObj(p.id);
                    //获得选中的节点
                    ...
                    for (var i = 0, l = nodes.length; i < l; i++) {
                        v += nodes[i].name + ",";
                        id += nodes[i].id;
                        getClick(nodes[i],that.element.name);

                    }
                    //将选中节点的名称显示在文本框内
                    if (v.length > 0) v = v.substring(0, v.length - 1);
                    var ztreeObj = $('#'+that.element.id);
                    ztreeObj.val(v);
                    ztreeObj.removeAttr('name');
                    ztreeObj.next().val(id);
                    //隐藏zTree
                    hideMenu();
                    return false;
                };
                ...
                //显示树
                function showMenu() {
                    var wrapperOffset = $('.content-wrapper').length>0 ? $('.content-wrapper').offset().top : 0;
                    var ztreeOffset = $("#"+that.element.id).parent('div').offset();
                    $('.'+p.id).css({ left: ztreeOffset.left + "px", top: ztreeOffset.top -wrapperOffset+ $("#"+that.element.id).height()+3+ "px" });
                };
                ...
                //搜索
                $(document).on('click','.'+p.id+' '+'.search-bt',function(){
                    var keyVal = $('.'+p.id+' '+'.searchZtree').val();
                    if (keyVal && keyVal.length > 0) {
                        InitialZtree();
                        var zTree = $.fn.zTree.getZTreeObj(p.id);
                        var nodeList = zTree.getNodesByParamFuzzy("name", keyVal.replace(/(^\s*)|(\s*$)/g, ""));
                        //将找到的nodelist节点更新至Ztree内
                        $.fn.zTree.init($("#"+p.id), setting, nodeList);
                    } else {
                        InitialZtree();                
                    } 
                });
                function InitialZtree() {
                    $.fn.zTree.init($("#"+p.id), setting, treeNode);
                };
